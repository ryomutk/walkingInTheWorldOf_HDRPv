...
namespace World.Data.Database
{
    ...
    public class RoadServer : Singleton<RoadServer>
    {
        ...
        Utility.Logger logger = new Utility.Logger("RoadLog", "ROAD SERVER");


        public RoadData GetRoadsByPath(Vector3 path, Vector3Int startDirectionVec, Vector3Int endDirectionVec, RoadType[] pathPattern = null)
        {
            return GetRoadsByPath(Vector3Int.RoundToInt(path), startDirectionVec, endDirectionVec, pathPattern);
        }
        public RoadData GetRoadsByPath(Vector3Int path, Vector3Int startDirectionVec, Vector3Int endDirectionVec, RoadType[] pathPattern = null)
        {
            return GetRoadsByPath(_defaultRoadSet, path, startDirectionVec, endDirectionVec, pathPattern);
        }

        /// <summary>
        /// ドアを原点としたローカル座標上で使えるRoadDataを返す。
        /// </summary>
        /// <param name="roadSet"></param>
        /// <param name="rawPath">"世界座標上での"path</param>
        /// <param name="startDirectionVec">出発点のdoorwayの向き。これは空いている向きと同じ。</param>
        /// <param name="endDirectionVec">目標のdoorwayの向いている方向。これは空いている方向の逆。</param>
        /// <param name="pathPattern"></param>
        /// <returns></returns>
        public RoadData GetRoadsByPath(IRoadSet roadSet, Vector3Int rawPath, Vector3Int startDirectionVec, Vector3Int endDirectionVec, RoadType[] pathPattern = null)
        {
            if (pathPattern == null)
            {
            ...
            var baseRotation = Quaternion.FromToRotation(startDirectionVec, Vector3.forward);
            var endDirectionVecRelative = baseRotation * endDirectionVec;
            var endDirection = World.Math.MathOfWorld.GetDirection(endDirectionVecRelative);
            var path = Vector3Int.RoundToInt(baseRotation * rawPath);
